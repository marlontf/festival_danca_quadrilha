 ...
 * 
 */
class modelInfantil extends model
{
	
	public function listarAll()
	{
		$sql = $this->pdo->prepare("SELECT * FROM tb_participantes WHERE id_categoria = 1");
		$sql ->execute();
		return $sql ->fetchAll();
	}
	...
	public function classificacaoCasamento()
	{
		$sql = $this->pdo->prepare("SELECT p.id, p.nome, 
			...
			FROM tb_quesitos_casamento AS c INNER JOIN tb_participantes AS p ON c.id_participante = p.id WHERE p.id_categoria = 1 GROUP BY c.id_participante 
	...
	public function classificacaoMarcador()
	{
		$sql = $this->pdo->prepare("SELECT p.id, p.nome, 
			...
			FROM tb_quesitos_marcador AS m INNER JOIN tb_participantes AS p ON m.id_participante = p.id WHERE m.id_categoria = 1 GROUP BY m.id_participante 
	...
	{
		$sql = $this->pdo->prepare("SELECT c.*, u.nome as jurado, p.* FROM tb_quesitos_quadrilha as c INNER JOIN tb_usuarios as u ON c.id_usuario = u.id INNER JOIN tb_participantes AS p ON p.id = c.id_participante WHERE c.id_categoria = '1' ORDER BY p.id");
		$sql ->execute();
		return $sql ->fetchAll();
	}
	...
	{
		$sql = $this->pdo->prepare("SELECT c.*, u.nome as jurado, p.* FROM tb_quesitos_casamento as c INNER JOIN tb_usuarios as u ON c.id_usuario = u.id INNER JOIN tb_participantes AS p ON p.id = c.id_participante WHERE c.id_categoria = '1' ORDER BY p.id");
		$sql ->execute();
		return $sql ->fetchAll();
	}
	...
	{
		$sql = $this->pdo->prepare("SELECT c.*, u.nome as jurado, p.* FROM tb_quesitos_marcador as c INNER JOIN tb_usuarios as u ON c.id_usuario = u.id INNER JOIN tb_participantes AS p ON p.id = c.id_participante WHERE c.id_categoria = '1' ORDER BY p.id");
		$sql ->execute();
		return $sql ->fetchAll();
	}
